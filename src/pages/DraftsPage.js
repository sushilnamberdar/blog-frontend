...
const DraftsPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      setPosts(res.data.posts.filter(post => post.status === 'draft') || []);
    } catch (err) {
  ...
  const handlePublish = async (postId) => {
    if (window.confirm('Are you sure you want to publish this post?')) {
      try {
        await axiosInstance.put(`/posts/${postId}`, { status: 'published' });
        showNotification('Post published successfully');
        fetchPosts(); // Refetch posts after publishing
      } catch (err) {
        showNotification(err.response?.data?.message || err.message || 'An error occurred', 'error');
        setError(err.response?.data?.message || err.message || 'An error occurred');
      }
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      <h1 className="text-3xl font-bold text-gray-900 mb-6 dark:text-gray-100">Drafts</h1>
      {showConfirmModal && (
        <ConfirmationModal
      ...
      {posts.length === 0 ? (
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p className="text-lg">You have no drafts.</p>
        </div>
      ) : (
          ...
          {posts.map(post => (
            <div key={post._id} className="relative">
              ...
              <div className="absolute top-2 right-2 flex space-x-2">
                ...
                <Button size="sm" variant="primary" onClick={() => handlePublish(post._id)}>Publish</Button>
              </div>
            </div>
          ))}
        </div>
